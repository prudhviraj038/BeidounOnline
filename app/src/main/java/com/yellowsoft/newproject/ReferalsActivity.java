...
public class ReferalsActivity extends AppCompatActivity implements PaymentResultListener {
	...
	protected void onCreate(@Nullable Bundle savedInstanceState) {
		...
		disclaimer_tv = (TextView)findViewById(R.id.disclaimer_tv);
		...
		try {
			disclaimer_tv.setText(Html.fromHtml(ApplicationController.getInstance().settings.getString("bank_disclaimer")));
		...
		final AlertDialog alertDialog = new AlertDialog.Builder(ReferalsActivity.this).create();
		...
		paynow_ll_referal.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(referal_code_global.equals("")){
					...
					alertDialog.setMessage("Enter referal code");
					alertDialog.show();
						...
						//Toast.makeText(ReferalsActivity.this,"Enter bank details",Toast.LENGTH_SHORT).show();

					//Snackbar.make(paynow_ll_referal,"Enter bank details or upi id",Snackbar.LENGTH_SHORT).show();
					alertDialog.setMessage("Enter bank details");
					alertDialog.show();

				}
				else if (ed_bank_ac.getText().toString().length()<8){
					alertDialog.setMessage("Account number should be minimum 8 characters");
					alertDialog.show();
				}
				else if (ed_ac_number_confirm.getText().toString().equals("")){
					alertDialog.setMessage("Confirm account number");
					alertDialog.show();
				}
				else if (!ed_bank_ac.getText().toString().equals(ed_ac_number_confirm.getText().toString())){

					//Toast.makeText(ReferalsActivity.this,"Enter same account number",Toast.LENGTH_SHORT).show();

					alertDialog.setMessage("Enter same account number");
					alertDialog.show();
				}
				else if (ed_ifsc_code.getText().toString().equals("")){
					alertDialog.setMessage("Please enter IFSC code");
					alertDialog.show();
				}
				else  if (ed_ifsc_code.getText().toString().length()<11){
					//Toast.makeText(ReferalsActivity.this,"IFSC code must be 11 characters",Toast.LENGTH_SHORT).show();
					alertDialog.setMessage("IFSC code must be 11 characters");
					alertDialog.show();
				}
